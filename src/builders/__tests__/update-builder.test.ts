...
describe("UpdateBuilder", () => {
  ...
  describe("execute", () => {
    ...
    it("should prepare updates before execute", async () => {
      const builder = new UpdateBuilder<TestItem>(mockExecutor, tableName, key).prepare({
        prepare: () => {
          builder.set("status", "prepared");
        },
      });
  ...
  describe("preparation hooks", () => {
    it("should prepare updates before withTransaction", () => {
      const transaction = {
      ...
      const builder = new UpdateBuilder<TestItem>(mockExecutor, tableName, key).prepare({
        prepare: () => {
          builder.set("status", "transaction-prepared");
        },
      });
      ...
      expect(transaction.updateWithCommand).toHaveBeenCalledWith(
        expect.objectContaining({
          updateExpression: "SET #0 = :0",
          expressionAttributeValues: {
            ":0": "transaction-prepared",
          },
        }),
      );
    });

    it("should prepare updates before debug", () => {
      const builder = new UpdateBuilder<TestItem>(mockExecutor, tableName, key).prepare({
        prepare: () => {
          builder.set("status", "debug-prepared");
        },
      });
    ...
    it("should not persist prepared updates across multiple renders", async () => {
      ...
      expect(firstDebug.raw.updateExpression).toBe("SET #0 = :0, #1 = :1");
      expect(secondDebug.raw.updateExpression).toBe("SET #0 = :0, #1 = :1");
