...
`ResultIterator` retains the most recent non-null `lastEvaluatedKey` returned by the executor for inspection, but `getContinuationKey()` returns it **only** when the most recent DynamoDB response says another page exists. Before fetching each later page it uses the source with `startFrom(lastEvaluatedKey)`; it ends when the response has no continuation or its overall yielded-item `limit` has been reached. That overall yield cap is distinct from the `Limit` sent to DynamoDB for a request. A retained diagnostic key must not be treated as permission to resume after the final response.

`Paginator` instead clones the source for each `getNextPage()`, applies the page size and its current continuation key, then advances page number and total retrieved count from `generator.getContinuationKey()`. A returned continuation key keeps `hasNextPage()` true; an absent one marks it exhausted, so later calls have no next page to fetch. It also reduces an effective page size when an overall limit leaves fewer items. `findOne()` clones with limit one but can traverse empty filtered pages. DynamoDB evaluates the operation limit before its filter, so filters can reduce returned matches below the requested count.
