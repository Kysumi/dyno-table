...
`error-utils.ts` exports AWS-name classifiers: conditional-check failure, transaction cancellation, validation exception, provisioned-throughput exceeded, and `isRetryableError`. `isConditionalCheckFailed` also recognizes that name on the `.cause` of an `OperationError`, preserving Table's AWS-error wrapping boundary for the [migration checkpoint retry](../migration-system.md). The retryable classification is an explicit name list: `ProvisionedThroughputExceededException`, `ThrottlingException`, `RequestLimitExceeded`, `InternalServerError`, and `ServiceUnavailable`.
